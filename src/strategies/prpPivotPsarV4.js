// PRP v4 — Pivot Reversal + PSAR (split filter timing / "cách 2")
// Direction confirm at arming bar; RVOL/MFI at breakout (trigger) bar only.
// Base: QuantNomad PRP Pine Script (@version=5)
//
// Core logic:
...
// ── Strategy Object ───────────────────────────────────────────────────────────
export const PrpPivotPsarV4Strategy = {
  id: "prp-pivot-psar-v4",
  name: "PRP - Pivot Reversal - PSAR (v4 arming/trigger filters)",
  ...
  generateSignals(candles, {
    ...
    const passesArmingDirection = (type, bar) =>
      passesDirectionConfirm(type, bar, directionConfirmMode, bodyBiasThreshold);

    const passesTriggerVolume = (type, i) => {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      if (i > 0) {
        if (longArmed && hprice > 0 && bar.high >= hprice + TICK) {
          ...
          if (canLong && passesTriggerVolume("long", i)) {
            signals.push({
              barIndex: i,
              time: bar.time,
              timestamp: bar.timestamp,
        ...
        if (shortArmed && lprice > 0 && bar.low <= lprice - TICK) {
          ...
          const entryPrice = bar.open < stopPrice ? bar.open : stopPrice;
          if (canShort && passesTriggerVolume("short", i)) {
            signals.push({
              barIndex: i,
              time: bar.time,
              timestamp: bar.timestamp,
      ...
      if (swl !== null) {
        ...
        if (zoneOk) {
          longArmed = passesArmingDirection("long", bar);
      ...
      if (swh !== null) {
        ...
        if (zoneOk) {
          shortArmed = passesArmingDirection("short", bar);
  ...
  getPendingLevels(candles, {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      if (swl !== null) {
        const zoneOk = !zoneFilterEnabled || isNearSupport(lprice, levels, ppLevels, zonePct);
        // v4: direction at arming; RVOL/MFI at trigger only (see generateSignals).
        if (zoneOk) {
          longArmed = passesDirectionConfirm("long", bar, directionConfirmMode, bodyBiasThreshold);
      ...
      if (swh !== null) {
        const zoneOk = !zoneFilterEnabled || isNearResistance(hprice, levels, ppLevels, zonePct);
        if (zoneOk) {
          shortArmed = passesDirectionConfirm("short", bar, directionConfirmMode, bodyBiasThreshold);
