// PRP v5 — Pivot Reversal + PSAR (intrabar 1m RVOL + trigger timing / "cách 3")
// Requires `generateSignals(candles, params, context)` with preloaded `context.minuteCandles`.
// Direction + MFI on TF trigger bar; RVOL from dynamic minute rows inside the TF bar.
//
// Core logic:
...
import { calcDynamicRvolForMinuteRows } from "../utils/rvolDynamicMinute.js";

let warnedPrpV5NoMinutes = false;

const BTCUSDT_MINTICK = 0.1;
const PRICE_EPS = 1e-9;

function lowerBoundByTimestamp(sortedCandles, ts) {
  let lo = 0;
  let hi = sortedCandles.length;
  while (lo < hi) {
    const mid = (lo + hi) >> 1;
    if (sortedCandles[mid].timestamp < ts) lo = mid + 1;
    else hi = mid;
  }
  return lo;
}
...
/**
 * First minute in [startMs, startMs+tfMs) where optional dynamic RVOL passes and price crosses trigger.
 */
function findIntrabarEntry({
  type,
  minuteCandles,
  tfCandles,
  startMs,
  ...
  const lo = lowerBoundByTimestamp(minuteCandles, startMs);
  const slice = [];
  for (let j = lo; j < minuteCandles.length && minuteCandles[j].timestamp < endMs; j++) {
    slice.push(minuteCandles[j]);
  }
  if (!slice.length) return null;

  const rvolArr = useRvol
    ? calcDynamicRvolForMinuteRows(slice, tfCandles, startMs, lookback)
    : null;

  const triggerLong = stopLevel + tick;
  const triggerShort = stopLevel - tick;

  for (let j = 0; j < slice.length; j++) {
    const m = slice[j];
    if (useRvol) {
      const rvol = rvolArr[j];
      if (rvol == null || rvol < rvolMin) continue;
    }
    if (type === "long") {
      if (m.close >= triggerLong) {
        return { entryPrice: m.close, timestamp: m.timestamp, time: m.time };
      }
    } else if (m.close <= triggerShort) {
      return { entryPrice: m.close, timestamp: m.timestamp, time: m.time };
    }
  }
  return null;
...
// ── Strategy Object ───────────────────────────────────────────────────────────
export const PrpPivotPsarV5Strategy = {
  id: "prp-pivot-psar-v5",
  name: "PRP - Pivot Reversal - PSAR (v5 intrabar 1m)",
  ...
  generateSignals(candles, {
    ...
    const minuteCandles = context.minuteCandles;
    const tfMinutes = Number(context.tfMinutes ?? 0);
    if (!minuteCandles?.length) {
      if (!warnedPrpV5NoMinutes) {
        console.warn("PRP v5: missing or empty context.minuteCandles — no signals.");
        warnedPrpV5NoMinutes = true;
      }
      return [];
    }
    warnedPrpV5NoMinutes = false;

    const tfMs = tfMinutes > 0 ? tfMinutes * 60 * 1000 : null;
    if (!tfMs) {
      console.warn("PRP v5: invalid context.tfMinutes — no signals.");
      return [];
    }
    ...
    // TF bar: direction + MFI only. RVOL uses intrabar dynamic series when filterMode is RVOL.
    const passesTfDirectionAndMfi = (type, i) => {
      const bar = candles[i];
      if (!passesDirectionConfirm(type, bar, directionConfirmMode, bodyBiasThreshold)) {
        return false;
      }
      if (!useMfi) return true;
      const mfi = mfiArr[i];
      const prevMfi = i > 0 ? mfiArr[i - 1] : null;
      if (type === "long") {
    ...
    for (let i = 0; i < candles.length; i++) {
      ...
      if (i > 0) {
        if (longArmed && hprice > 0 && bar.high >= hprice + TICK) {
          if (canLong && passesTfDirectionAndMfi("long", i)) {
            const hit = findIntrabarEntry({
              type: "long",
              minuteCandles,
              tfCandles: candles,
              startMs: bar.timestamp,
              tfMs,
              lookback: Number(rvolLookback ?? 0),
              rvolMin: Number(rvolMin ?? 0),
              useRvol,
              stopLevel: hprice,
              tick: TICK,
            });
            if (hit) {
              signals.push({
                ...
                stopLevel: hprice,
                exitFn: makeExitFn("long", hit.entryPrice),
              });
            }
          }
          longArmed = false;
        ...
        if (shortArmed && lprice > 0 && bar.low <= lprice - TICK) {
          if (canShort && passesTfDirectionAndMfi("short", i)) {
            const hit = findIntrabarEntry({
              type: "short",
              minuteCandles,
              tfCandles: candles,
            ...
            if (hit) {
              signals.push({
                ...
                entryPrice: hit.entryPrice,
                stopLevel: lprice,
                exitFn: makeExitFn("short", hit.entryPrice),
              });
            }
          }
          shortArmed = false;
