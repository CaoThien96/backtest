...
Fetches Bybit candles from startTime to now for a timeframe, runs PivotReversal, and prints signal-color alignment stats.

Usage:
  node ./backtest_pivot.js --startTime <ms> --timeframe <5|15|30|60|D> [options]
...
Example:
  node ./backtest_pivot.js --startTime 1742688000000 --timeframe 30 --leftBars 3 --rightBars 3 --minTick 1
`.trim();
}
...
async function fetchBybitKlines({ symbol, timeframe, startTime, endTime }) {
  ...
  while (cursorEnd >= startTime) {
    ...
    if (!res.ok) {
      throw new Error(`Bybit API HTTP ${res.status}`);
    }
...
function computeAlignmentStats(signals, candles) {
  let longTotal = 0;
  let longAligned = 0;
  let shortTotal = 0;
  let shortAligned = 0;
  ...
  for (const sig of signals) {
    const bar = candles[sig.barIndex];
    if (!bar) continue;
    if (sig.type === "long") {
      longTotal += 1;
      if (bar.close >= bar.open) longAligned += 1;
    } else if (sig.type === "short") {
      shortTotal += 1;
      if (bar.close <= bar.open) shortAligned += 1;
    }
  }
...
async function main() {
  ...
  const stats = computeAlignmentStats(signals, candles);

  console.log(JSON.stringify(stats, null, 2));
}

main().catch((err) => {
  console.error(`[backtest_pivot] ${err?.message ?? err}`);
  process.exitCode = 1;
});
